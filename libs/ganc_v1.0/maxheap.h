...
struct tuple {
	...
	double	  m2;					// second stored value (ajdacency)
...
public:
	...
	void		updateItem(tuple *address, double newStored,double newStored2);// update only the stored value of tuple at address 
...
tuple* maxheap::insertItem(const tuple newData) {			// O(log k) time
	...
	A[index].d->m2  = newData.m2;		// add newData onto the bottom of the heap
...
void maxheap::updateItem(tuple *address, tuple newData) {   // O(log k) time
	double oldm = address->m,oldm2 = address->m2;
	address->m = newData.m;						//   udpate the dQ stored
	address->m2 = newData.m2;						//   udpate the adjacency stored
...
void maxheap::updateItem(tuple *address, double newStored, double newStored2) {	// O(log k) time
	double oldm = address->m, oldm2 = address->m2;
	address->m	= newStored;						// udpate the dQ stored
	address->m2	= newStored2;						// udpate the dQ stored
...
void maxheap::deleteItem(tuple *address) {
	...
	if (heaplimit==2) {						// check if deleting last item in heap
		...
		A[1].d->m2		= -4294967296.0;		// zero out root data
		A[1].d->i		= 0;					// 
		A[1].d->j		= 0;					// 
		A[1].d->k		= 1;					// 
		...
		A[index].d->m2  = -4294967296.0;		// zero out the deleted item's data
		A[index].d->i	= 0;					// 
		A[index].d->j  = 0;					// 
...
void maxheap::printHeap() {
	for (int i=1; i<heaplimit; i++) {
		cout << A[i].d <<"\t["<<A[i].d->k<<"]\tdQ = "<<A[i].d->m<<"\t"<<A[i].d->i<<" -> "<<A[i].d->j<<"\t\t"<<A[i].d->m2<<"\n"; }
	return;
}
