// This file is slightly modified by Seyed Salim Tabatabaei 
// to include the adjacency matrix (seyed.s.tabatabaei@mail.mcgill.ca)
...
public:
	...
	double stored2;					// to store adjacency
...
element::element()  {    key = 0; stored = -4294967296.0; stored2 = 0; color = false;
...
public:
	...
	void		insertItem(int newKey, double newStored, double newStored2);		// insert a new key with stored value
...
tuple vektor::returnMaxKey() {
	...
	themax.m2 = current->stored2;			// store the data found
...
void vektor::insertItem(int newKey, double newStored, double newStored2) {
	...
	if (current != NULL) {
		...
		current->stored2 += newStored2;					// update its stored value: A_jx = A_jx + A_ix
		current->stored = newStored;					// update its stored value
		//current->stored2 = newStored2;					// update its stored value
		heap->updateItem(current->heap_ptr, current->stored, current->stored2);
...
//	cout<<"Node "<<killKey<<" removed"<<endl;
//	cin.get();
	element *x, *y, *z;
	char pauseme;
